...
import { useRealm, masterUserQuery, IUserSchema, createAccount, updateUser, updateWallet, getActiveWallet, createWallet, IAddressSchema, networkNameTypes } from '../../../../realm';
...
const ChooseAddress = () => {
    ...
    const { passphrase, networkName } = useLocalSearchParams();
    const handleSubmit = async () => {
        try {
            if (!passphrase || !networkName) {
                console.log('no passphrase or networkName')
                // router.back()
                return
            }
            const wallet = await createWallet({
                passphrase: passphrase as string,
                networkName: networkName as networkNameTypes
            });
            const user = masterUserQuery();
            const data = await createAccount({ privateKey: 'privateKey', addresses: availableAddresses, walletId: wallet!._id, selectedAddressIndex });
            updateWallet(wallet!._id, { activeAccountId: data._id, activeNetworkName: networkName as networkNameTypes });
            router.push(appRoutes.tabs.home);
        } catch (error) {
                        ...
                        >
                            ...
                            <View style={{ flex: 1, flexDirection: 'column', alignItems: 'center', gap: 10, justifyContent: 'center' }}>
                                ...
                                >
                                    ...
                                    <TouchableOpacity onPress={() => { console.log('copied') }} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, width: width * 0.5 }}>
